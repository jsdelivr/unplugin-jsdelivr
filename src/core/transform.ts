...
const generateImportTuple = (importElem: string): ImportTuple => {
  ...
  if (importRename.length === 1) return [importRename[0], importRename[0]]; // 'map' into ['map', 'map']

  return [importRename[0], importRename[1]]; // 'merge as LodashMerge' into ['merge', 'LodashMerge']
};

const generateImportStatement = async (
  importModule: string,
  importTuple: ImportTuple,
  transform: ModuleMapReturn,
  ctx: Context
): Promise<string> => {
  ...
  const transformedModule =
    typeof transform === 'string'
      ? `${transform}@${version}`
      : transform(`${importModule}@${version}`, originalImport);
...
const updateCode = (code: string, importStatements: ImportStatementTuple[]) => {
  ...
  for (const importStatementTuple of importStatements) {
    const [importStatement, statementStart, statementEnd] =
      importStatementTuple;
...
const transformImports = async (code: string, ctx: Context) => {
  ...
  for (const importSpecifier of imports) {
    ...
    if (transformFunction !== undefined) {
      ...
      const importElems = importStatement
